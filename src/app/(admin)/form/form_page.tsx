...
type ExMode = Mode | (string & {})
type ExVa = Exclude<Mode, "onTouched" | "all"> | (string & {})

type FormPageProps = {
  mode: ExMode;
  setMode: React.Dispatch<React.SetStateAction<ExMode>>
  reValidateMode: ExVa
  setReValidateMode: React.Dispatch<React.SetStateAction<ExVa>>

}
function FormPage({ mode, reValidateMode, setMode, setReValidateMode }: FormPageProps) {
  ...
  const methods = useForm<FormValues>({
    defaultValues: {
      ...
      cardNumber: "",
      country: "",
      daily_rate: "" as unknown as number,
      daily_rate_half: "" as unknown as number,
      daily_weekend_rate: "" as unknown as number,
      daily_weekend_rate_half: "" as unknown as number,
      hour_rate: "" as unknown as number,
      hour_rate_half: "" as unknown as number,
      hour_weekend_rate: "" as unknown as number,
      hour_weekend_rate_half: "" as unknown as number,
    },
    resolver,
    mode: mode as Mode,
  ...
  const { control, reset, register, handleSubmit, watch } = methods;
  ...
  return (
    <FormProvider  {...methods}>
      <div className=" container mx-auto space-y-3 pb-20 max-w-5xl">
        <CardTitle className="sticky top-[1rem] z-[1] bg-white/35 h-12 flex items-center justify-center text-xl ">
          Parent Render Count {render.current}
        </CardTitle>
        <Card>
          <CardHeader className="space-y-4">
            <div className="space-y-4">
              <Alert variant="default" className="leading-5 text-lg !z-[0]">
                <p>
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    {["onBlur", "onSubmit", "onChange"].map((value) => (
                      <SelectItem key={value} value={value}>
                        {value}
                      </SelectItem>
                    ))}
...
export default function Parent() {
  const [mode, setMode] = useState<ExMode>("onTouched");
  const [reValidateMode, setReValidateMode] = useState<ExVa>("onBlur");

  return (
    <>
      <FormPage
        key={`${mode}_${reValidateMode}`}
        mode={mode}
        setMode={setMode}
        reValidateMode={reValidateMode}
        setReValidateMode={setReValidateMode}
      />
    </>
  )
}